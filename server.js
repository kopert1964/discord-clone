...
app.post('/api/register', async (req, res) => {
  ...
  if (users.find(u => u.username === username)) return res.status(400).json({ error: 'Ник уже занят' });
  
  const hashedPassword = await bcrypt.hash(password, 10);
  const user = { id: uuidv4(), username, email, password: hashedPassword, avatar: null, status: 'online', createdAt: Date.now() };
  users.push(user);
...
app.get('/api/users', (req, res) => {
  res.json(users.map(u => ({ id: u.id, username: u.username, avatar: u.avatar, status: u.status })));
});
...
app.get('/api/messages/:channelId', (req, res) => {
  res.json(messages.filter(m => m.channelId === req.params.channelId).slice(-100));
});
...
io.on('connection', (socket) => {
  ...
  socket.on('auth', ({ token }) => {
    ...
    } catch (e) { console.log('❌ Неверный токен сокета'); }
  });
  
  socket.on('send-message', ({ channelId, text }) => {
    if (!currentUser || !text?.trim()) return;
    const message = { id: uuidv4(), channelId, userId: currentUser.id, username: currentUser.username, avatar: currentUser.avatar, text: text.trim(), edited: false, createdAt: Date.now() };
  ...
  socket.on('leave-voice', () => { leaveAllVoiceChannels(socket); });
  ...
  socket.on('send-dm', ({ to, text }) => {
    ...
    const message = { id: uuidv4(), userId: currentUser.id, username: currentUser.username, avatar: currentUser.avatar, text: text.trim(), createdAt: Date.now() };
